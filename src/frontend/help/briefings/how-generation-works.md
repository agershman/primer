...
Briefings generate automatically via cron at **05:00 UTC, every day**, with the briefing's calendar `briefing_date` stamped using the user's stored timezone (see [Timezones and "today"](#timezones-and-today)). A Sunday maintenance job runs at **03:00 UTC** to handle concept decay, retention sweeps, and reaping stuck notifications. You can also trigger an on-demand run at any time via the **Generate now** action at the top of the feed — useful after a particularly active day or when you want fresh content. Generate now is additive: it appends any new pieces to the log, never replaces what's already there.

If a given day has nothing worth surfacing — no new work signals, no adjacent reading material, no decaying concepts — the generator finalizes the briefing row with zero teaching pieces and a structured `metadata.reason` (`no_candidates`). The `/briefing/today` endpoint promotes that as `noContentReason`. The frontend reads this on the completion of an on-demand run and surfaces it as a quiet toast (plus the existing bell notification) rather than replacing the feed — the user's prior content stays put.

There's a distinct bucket for the case where the generator drafted pieces but the continuation classifier matched every one against a recent predecessor: `all_drafts_redundant`. Without this, an additive run that did real work but filtered everything as overlap would surface the misleading "nothing surfaced" copy. The dedicated bucket lets the toast read "We drafted some pieces, but they overlapped with recent teaching" — accurate, and matches what the redundantDrafts metadata already records server-side.

Failure paths (`all_pieces_failed`, `monthly_budget_exceeded`) surface as a negative-tone toast on the same completion event.
