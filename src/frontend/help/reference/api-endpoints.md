...
|--------|------|-------------|
| `GET` | `/api/briefing/today` | Get today's briefing with pieces and pending quiz. Each piece includes `source_context` (JSON array of source descriptors with `type`, `title`, `url`, `dueAt`, `dueReason`) plus optional `series_id` + `part_number` (set when the piece is in a multi-part series). The briefing object includes `redundantDrafts` — an array of entries describing topics the continuation classifier filtered as redundant during generation, each with `predecessor_id`, `predecessor_title`, `predecessor_briefing_date`, `predecessor_series_id`, `predecessor_part_number`, and `reason`. Empty array when nothing was filtered. The briefing object also includes `noContentReason` — set on a finalized briefing with zero teaching pieces (`no_candidates` / `all_pieces_failed` / `all_drafts_redundant` / `monthly_budget_exceeded` / `cancelled` / `unknown`). `null` while the briefing is generating or when pieces > 0. The frontend reads this on Generate-now completion to decide which toast copy to fire (`all_drafts_redundant` gets "we drafted some pieces but they overlapped with recent teaching"; `no_candidates` gets "nothing new surfaced"). |
...
| `GET` | `/api/analytics/budget` | Calendar-month-to-date spend vs. the user's monthly cap, scoped to `created_at >= start of month` so the number matches what `isBudgetExceeded()` compares against when halting generation. Returns `{ cap, spend, monthStart, byOperation: [{ operation, modality, costUsd, calls }], byProvider: [{ provider, costUsd, calls }] }` — both arrays sorted by `costUsd` desc. Powers the "battery usage"–style display inside the Briefing limits settings panel. |
